...
/*! definimos la cantidad de usuarios maxima */
#define MAX_BT_USERS	7
...
void *process_client(void *n)
{
	char buff[1024] = {0};
	int bytes_read = 1, leftToRead = 15;
	int client = *((int *)n);
	FILE *file = NULL;
	
	sprintf(buff, "dispoistivo%d",client);
	file = fopen(buff, "w");
	//[B@1a4cfaaa
	bytes_read = send(client, "resp:36:00:1A:7D:01:54:6A,11:11:11:11:11:11,", 
			   strlen("resp:36:00:1A:7D:01:54:6A,11:11:11:11:11:11,"), 0);	
	printf("Enviamos:%d bytes al celu\n", bytes_read);
	if (bytes_read < 0)
		printf("Error al mandar datos al celu\n");
...
static void set_discoverable(char *mac)
{
	int result = 0;
	int dd, ctl, err;
	...
	result = hci_devid(mac);
	
	if (result < 0) {
		perror ("Error al obtener el devid");
		return;
	}
	
	ctl = hci_open_dev(result);
	...
	dr.dev_id = (uint16_t) result;
	if (ioctl(ctl, HCISETSCAN, (void *) &dr) < 0) {
...
int main(int argc, char **argv)
{
    ...
    char buff[1024] = {0};
    int s = 0, client[MAX_BT_USERS] = {0}, bytes_read = 1;
    unsigned int opt = sizeof(rem_addr);
    sdp_session_t * session = NULL;
    int leftToRead = 0, i = 0;
    pthread_t clientThread[MAX_BT_USERS];
    
    //str2ba("00:1A:7D:01:54:6A", &localDevice);
    str2ba("11:11:11:11:11:11", &localDevice);
    set_discoverable("11:11:11:11:11:11");
    register_service();
    session = register_service();
    /* creamos el socket */
    ...
    bacpy(&loc_addr.rc_bdaddr, &localDevice);
    ...
    while (i < MAX_BT_USERS) {
	printf("Esperando clientes.... \n");
	client[i] = accept(s, (struct sockaddr *)&rem_addr, &opt);
	ba2str(&rem_addr.rc_bdaddr,buff);
	printf("Aceptamos conexion de: %s\n", buff);
	memset(buff, 0, sizeof(buff));
	pthread_create(&(clientThread[i]),NULL,process_client,(void *)&client[i]);
	i++;
	
    }
    printf("\nesperando a ver si recibimos algo mas\n");
    sleep(5);
    for (i = 0; i < MAX_BT_USERS; i++) {
	    close(client[i]);
    }
    close(s);
    sdp_close(session);
