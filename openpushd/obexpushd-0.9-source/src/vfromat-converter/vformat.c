...
/* This file has been stolen from the vformat plugin of OpenSync. */
...
static size_t base64_encode_step(unsigned char *in, size_t len, gboolean
break_lines, unsigned char *out, int *state, int *save);
static size_t base64_decode_step(unsigned char *in, size_t len, unsigned char
*out, int *state, unsigned int *save);
...
upper_line = g_ascii_strup(line->str, -1);
...
static void _read_attribute_value_add (VFormatAttribute *attr, GString *str,
GString *charset)
{
	char *inbuf, *outbuf, *p;
	iconv_t cd;
...
cd = iconv_open("UTF-8", charset->str);
                ...
                if (iconv(cd, (const char**)&inbuf, &inbytesleft, &p,
&outbytesleft) != (size_t)(-1)) {
#else
                if (iconv(cd, &inbuf, &inbytesleft, &p, &outbytesleft) !=
(size_t)(-1)) {
#endif
                        *p = 0;
...
static void _read_attribute_value (VFormatAttribute *attr, char **p, int
format_encoding, GString *charset)
{
char *lp = *p;
GString *str;
GString *str_nocr;
...
str = g_string_append_c (str, '\\');
// bypass the following g_utf8_next_char : we need to keep this "=" for the next
//loop run if QP
continue;
}
default:
...
static void _read_attribute_params(VFormatAttribute *attr, char **p, int
*format_encoding, GString **charset)
{
char *lp = *p;
GString *str;
...
else if (in_quote || g_unichar_isalnum (g_utf8_get_char (lp)) || *lp == '-' ||
*lp == '_' || *lp == '/' || *lp == '.' || *lp == ' ') {
str = g_string_append_unichar (str, g_utf8_get_char (lp));
lp = g_utf8_next_char (lp);
}
...
if (param && !comma) {
if(g_ascii_strcasecmp (param->name, "encoding")) // value are already decoded in
//_read_attribute_value. Setting encoding
vformat_attribute_add_param (attr, param);// would lead to double decoding of
//the value.
param = NULL;
}
if (colon)
...
break;
case ',':
if (type == VFORMAT_CARD_30 || type == VFORMAT_EVENT_20 || type ==
VFORMAT_TODO_20)
str = g_string_append (str, "\\,");
else
...
static VFormatAttribute *vformat_find_attribute(VFormat *vcard, const char *name)
{
...
**/
must_have_type = FALSE;
if (!g_ascii_strcasecmp (attr->name, "PHOTO") || !g_ascii_strcasecmp
(attr->name, "LOGO") || !g_ascii_strcasecmp (attr->name, "SOUND") )
must_have_type = TRUE;
if ( must_have_type || g_ascii_strcasecmp (param->name, "TYPE") )
attr_str = g_string_append (attr_str, param->name);
if ( param->values && (must_have_type || g_ascii_strcasecmp (param->name,
"TYPE")) )
attr_str = g_string_append_c (attr_str, '=');
for (v = param->values; v; v = v->next) {
...
*
* SUMMARY: When generating a content line, lines longer then 75 characters
SHOULD be folded!
*
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
^^^^^^^^^^
...
* The quoted-printable specs says that softbreaks should be generated by
inserting a =\r\n
* without follwing <WS>
*
* UTF-8
...
else if (g_utf8_get_char(g_utf8_offset_to_pointer(attr_str->str, l-2)) == '=') l
-= 2;
}
 
p1 = g_utf8_offset_to_pointer(attr_str->str, l);
 
if (format_encoding == VF_ENCODING_QP)
attr_str = g_string_insert_len (attr_str, p1 - attr_str->str, "=" CRLF "", sizeof
("=" CRLF "") - 1);
else
attr_str = g_string_insert_len (attr_str, p1 - attr_str->str, CRLF " ", sizeof
(CRLF " ") - 1);
...
VFormatAttribute *vformat_attribute_new (const char *attr_group, const char
*attr_name)
{
VFormatAttribute *attr;
...
void
vformat_remove_attributes (VFormat *evc, const char *attr_group, const char
*attr_name)
{
GList *attr;
...
void
vformat_attribute_add_value_decoded (VFormatAttribute *attr, const char *value,
int len)
{
g_return_if_fail (attr != NULL);
...
case VF_ENCODING_QP: {
char *qp_data = quoted_encode_simple ((const unsigned char*)value, len);
...
VFormatParam *vformat_attribute_find_param(VFormatAttribute *attr, const char
*name)
{
	GList *p = NULL;
g_return_val_if_fail (attr != NULL, NULL);
...
void
vformat_attribute_add_param_with_value (VFormatAttribute *attr, const char
...
param = vformat_attribute_param_new(name);
...
attr->decoded_values = g_list_append (attr->decoded_values, g_string_new
((char*)l->data));
break;
case VF_ENCODING_BASE64:
for (l = attr->values; l; l = l->next) {
...
attr->decoded_values = g_list_append (attr->decoded_values, g_string_new_len
(decoded, len));
g_free (decoded);
}
break;
...
if (!(l->data))
continue;
decoded = g_strdup ((char*)l->data);
len = quoted_decode_simple (decoded, strlen (decoded));
attr->decoded_values = g_list_append (attr->decoded_values, g_string_new_len
(decoded, len));
g_free (decoded);
}
...
static size_t base64_encode_close(unsigned char *in, size_t inlen, gboolean
break_lines, unsigned char *out, int *state, int *save)
{
int c1, c2;
